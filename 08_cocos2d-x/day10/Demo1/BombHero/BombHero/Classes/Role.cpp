...
bool Role::initRole() {
    ...
    m_showSprite->setAnchorPoint(ccp(0.5, 0));
...
CCAnimate* Role::createAnimate(const char *spriteFramName, int frameCount, int Loops) {
...
void Role::addWillDieAnimate(std::string frameName) {
        CCAnimate *animate = this->createAnimate(frameName.c_str(), 1, 0);
...
void Role::runRole_WillDie() {
    m_rolestate = Role_WillDie;
    m_showSprite->stopAllActions();
    //从字典中取出 将要死亡动画对象
    CCAnimate *animate = this->getAnimate(Role_WillDie, Role_Down);
    CCBlink *blink = CCBlink::create(1.2, 6);
    CCSpawn *spawn = CCSpawn::create(animate, blink, NULL);
    CCCallFunc *callFunc = CCCallFunc::create(this, callfunc_selector(Role::Role_DidDie));
    CCSequence *seq = CCSequence::create(spawn, callFunc, NULL);
    m_showSprite->runAction(seq);
}
//角色已经死亡
void Role::Role_DidDie() {
    m_rolestate = Role_Die;
    CCLog("角色已经死亡");
}
...
void Role::setCollisionArea(CCRect var) {
    m_collisionArea = var;
}

CCRect Role::getCollisionArea(){
    return CCRectMake(m_collisionArea.origin.x + this->getPositionX(),m_collisionArea.origin.y + this->getPositionY(), m_collisionArea.size.width, m_collisionArea.size.height);
}
