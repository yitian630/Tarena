...
void Rocker::ccTouchesBegan(CCSet *pTouches, CCEvent *pEvent){
    ...
    while (iter != pTouches->end()) {
        ...
        if (this->isTouchInside(touch)) {
            ...
            //如果 任意方向的 被委托者存在
            if (m_delegate_Custom) {
                m_delegate_Custom->touchBegan_cus(this, this->getNormal(point, CCPointZero));
            }
...
void Rocker::ccTouchesMoved(CCSet *pTouches, CCEvent *pEvent){
        if (tempTouch) {
            ...
            //如果 任意方向的 被委托者存在
            if (m_delegate_Custom) {
                m_delegate_Custom->touchMove_cus(this, this->getNormal(point, CCPointZero));
            }
    }
}
void Rocker::ccTouchesEnded(CCSet *pTouches, CCEvent *pEvent){
        if (tempTouch) {
            ...
            //如果 任意方向的 被委托者存在
            if (m_delegate_Custom) {
                m_delegate_Custom->touchEnd_cus(this);
            }
