...
bool SelectMapLayer::init() {
    ...
    //每个地图关之间的间隔距离
    float space = map_space;
    for (int i = 0; i < m_maxMapPage; i ++) {
        //创建每大关地图选项
        SelectMapNode *mapNode = SelectMapNode::createSelectMapNode(i + 1);
        mapNode->setPosition(ccp((mapNode->getShowContentSize().width * 0.5 + space) + (mapNode->getShowContentSize().width + space) * i, 0));
        //讲节点 添加  内容的层中
        m_containerLayer->addChild(mapNode);
        if (i == m_maxMapPage - 1) { //最后一个精灵
            //每页的地图宽度
            m_mapPageWidth = mapNode->getShowContentSize().width + space;
            float container_W = (mapNode->getShowContentSize().width * 0.5 + space) * 2 + m_mapPageWidth * m_maxMapPage;
    ...
    m_scrollView->setContentSize(m_containerLayer->getContentSize());//设置scrollview区域的大小
