...
-(void)getUserTimelineByName:(NSString *)name complation:(MyCallback)complation{
 
    NSString *path = [NSString stringWithFormat:@"%@statuses/user_timeline",BASE_URL];
    NSString *params = [NSString stringWithFormat:@"%@&format=json&name=%@&listid=0&reqnum=20&pageflag=0&type=0&contenttype=0&pageTime=0",self.normalParams,name];
    NSURL *url = [NSURL URLWithString:path];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setHTTPMethod:@"POST"];
    [request setHTTPBody:[params dataUsingEncoding:NSUTF8StringEncoding]];
