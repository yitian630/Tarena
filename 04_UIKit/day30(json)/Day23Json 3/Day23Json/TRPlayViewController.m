...
- (void)viewDidLoad
{
    ...
    self.keys = self.lrcDic.allKeys;
    self.keys = [self.keys sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data{
    ...
    if (self.musicData.length>100*1024&&!self.player) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:.1 target:self selector:@selector(checkingTimeAction) userInfo:Nil repeats:YES];
    }
}

-(void)checkingTimeAction{
  
    
    for (int i=0; i<self.keys.count; i++) {
        NSNumber *key = self.keys[i];
